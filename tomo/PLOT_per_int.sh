...
do
    ...
    echo 38.75 -14 ''${per[$i]}'s' | gmt pstext -J$J -R$R -Gwhite -C0.1c/0.1c -W1p -F+f17p -O -K >> $PS
    avg=`awk -F ',' '$1=='${per[$i]}' {print $2}' ../output_tomo/avgvel.dat`
    echo 27.75 3 ''$avg' km/s' | gmt pstext -J$J -R$R -Gwhite -C0.1c/0.1c -W1p -F+f17p -O -K >> $PS

    DSCALE=1.5i/-0.2i/2.6i/0.1ih
	gmt psscale -C$CPT -D$DSCALE -O -K -X0 -B+l"@~\144@~C@-R@-/C@-R@- (%)" >> $PS
